...
import * as model from './datepicker-view-model';
...
export class NgbDatepickerMonthView {
  ...
  @Input() month: model.MonthViewModel;
  ...
  doSelect(day: model.DayViewModel) {
    if (!day.context.disabled && !this.isHidden(day)) {
      this.select.emit(NgbDate.from(day.date));
    }
  }

  isCollapsed(week: model.WeekViewModel) {
    return this.outsideDays === 'collapsed' && week.days[0].date.month !== this.month.number &&
      week.days[week.days.length - 1].date.month !== this.month.number;
  }

  isHidden(day: model.DayViewModel) {
