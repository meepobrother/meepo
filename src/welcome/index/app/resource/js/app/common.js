    ...
    $(".panes").size() > 0 && require(["hammer"], function(t) {
        ...
        e.prototype = {
            show: function(a, e, i, n) {
                ...
                var o, d, c;
                for (o = 0; o < this.panes.length; o++)
                    if (d = this.containerSize / 100 * (100 * (o - a) + i), c = this.direction & t.DIRECTION_HORIZONTAL ? "translate3d(" + d + "px, 0, 0)" : "translate3d(0, " + d + "px, 0)", this.panes[o].style.transform = c, this.panes[o].style.mozTransform = c, this.panes[o].style.webkitTransform = c, a != e && (e == o && (l = $(".pane").eq(o).find("div[type]")).each(function() {
                            var t = $(this).attr("type").substring(0, 3);
                            if ("adi" === t) a = $(this).find("div[class='app-adImg']");
                            else var a = $(this).find("div[class^='app-" + t + "']");
            ...
            onPan: function(t) {
                if (1 == $(".pane").size()) {
                    ...
                    var e = $(".panes").parent().css("height"),
                        r = t.deltaY - i;
                    if (n += r, (s -= r) < 0) return s = 0, n = 0, i = 0, !1;
                    if (s + 568 > parseInt(e)) return s = parseInt(e) - 568, n = -(parseInt(e) - 568), i = 0, !1;
                    if (parseInt(e) > Math.abs(s) && s >= 0) {
                        var o = $(".alock");
                        if (o.length > 0)
                            for (var d = 0; d < o.length; d++) $(o.get(d)).parent("div").css({ transform: "translate3d(0, " + -n + "px, 0)", "-moz-transform": "translate3d(0, " + -n + "px, 0)", "-webkit-transform": "translate3d(0, " + -n + "px, 0)", "z-index": "2" });
                        $(".pane").css({ transform: "translate3d(0, " + -s + "px, 0)", "-moz-transform": "translate3d(0, " + -s + "px, 0)", "-webkit-transform": "translate3d(0, " + -s + "px, 0)", "z-index": "1" })
                    }
                    i = t.deltaY ? t.deltaY : 0, "panend" != t.type && "pancancel" != t.type || (i = 0)
                } else {
                    var c = a(this.direction, t.deltaX, t.deltaY),
                        l = 100 / this.containerSize * c,
                        h = !1,
                        p = this.currentIndex;
                    "panend" != t.type && "pancancel" != t.type || (Math.abs(l) > 10 && "panend" == t.type && (this.currentIndex += l < 0 ? 1 : -1), l = 0, h = !0), this.show(this.currentIndex, p, l, h)
                }
            }
        }, new e(document.querySelector(".panes"), t.DIRECTION_VERTICAL).hammer.get("pan"), $(window).on("scroll.elasticity", function(t) { t.preventDefault() }).on("touchmove.elasticity", function(t) { t.preventDefault() }), $(window).delegate("img", "mousemove", function(t) { t.preventDefault() })
    })
});